...
const NivelForm = ({ nivelToEdit, onSubmit, onCancel }) => {
  ...
  useEffect(() => {
    if (nivelToEdit) {
      setFormData({
        codigonivel: (nivelToEdit.codigonivel || "").trim(),
        nivel: (nivelToEdit.nivel || "").trim(),
        codigoinmueble: nivelToEdit.codigoinmueble != null ? String(nivelToEdit.codigoinmueble) : "",
        estado: nivelToEdit.estado != null ? String(nivelToEdit.estado) : "",
        codigoinstitucion: (nivelToEdit.codigoinstitucion || "").trim(),
        login_sid: nivelToEdit.login_sid || "",
      });
    } else {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const newErrors = {};
    if (!formData.nivel.trim()) newErrors.nivel = "El nombre del nivel es requerido";
    if (Object.keys(newErrors).length > 0) { setErrors(newErrors); return; }
    setIsSubmitting(true);
    try {
      const submitData = { ...formData };
      ["codigoinmueble", "estado"].forEach(f => { if (submitData[f] === "") submitData[f] = null; });
  ...
  const isEditing = Boolean(nivelToEdit);
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 py-4">
      ...
      <div className="flex justify-end space-x-2 pt-4">
        ...
        <Button type="submit" disabled={isSubmitting || !formData.nivel.trim()}>
          {isSubmitting ? "Procesando..." : nivelToEdit ? "Actualizar" : "Guardar"}
        </Button>
      </div>
    </form>
  );
