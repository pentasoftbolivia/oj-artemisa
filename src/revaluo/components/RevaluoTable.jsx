  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3 sm:hidden">
        {activos.map((a) => (
          <div key={String(a.codigoActivoInterno ?? a.codigoactivointerno ?? a._codigoActivo)} className="rounded-lg border bg-card p-3 space-y-3 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="space-y-1">
                <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Valor / Conservación</div>
                <div className="font-mono text-xs font-semibold">{a._valorActual ?? "—"}</div>
                <span className={`inline-flex px-2 py-0.5 rounded text-[11px] font-medium ${String(a._estadoConservacion || "").toUpperCase() === "BUENO" ? "bg-green-100 text-green-800" : String(a._estadoConservacion || "").toUpperCase() === "REGULAR" ? "bg-yellow-100 text-yellow-800" : String(a._estadoConservacion || "").toUpperCase() === "MALO" ? "bg-red-100 text-red-800" : "bg-muted text-muted-foreground"}`}>{a._estadoConservacion || "—"}</span>
              </div>
            </div>
            <div className="text-xs space-y-1">
              <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Inventariador</div>
              <div className="truncate text-muted-foreground">{a._inventariador || "—"}</div>
            </div>
            {a.observaciones ? (
              <div className="text-xs space-y-1">
            ...
            {activos.map((a) => (
              <TableRow key={String(a.codigoActivoInterno ?? a.codigoactivointerno ?? a._codigoActivo)}>
                ...
                <TableCell className="text-xs">
                  <span
                    className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${String(a._estadoConservacion || "").toUpperCase() === "BUENO" ? "bg-green-100 text-green-800" : String(a._estadoConservacion || "").toUpperCase() === "REGULAR" ? "bg-yellow-100 text-yellow-800" : String(a._estadoConservacion || "").toUpperCase() === "MALO" ? "bg-red-100 text-red-800" : "bg-muted text-muted-foreground"}`}
                  >
                ...
                <TableCell className="text-center">
                  {(() => {
                    ...
                    const label = ESTADO_MAP[v] ?? ESTADO_MAP[key] ?? ESTADO_MAP[upper] ?? (key ? (upper === "ALTA" ? "Alta" : upper === "BAJA" ? "Baja" : key) : "—");
                    const isAlta = label === "Alta";
                    return <Badge variant={isAlta ? "secondary" : "destructive"} className={isAlta ? "bg-yellow-400 text-yellow-900 border-yellow-500 hover:bg-yellow-500 text-xs" : "bg-red-600 text-white border-red-600 hover:bg-red-700 text-xs"}>{label}</Badge>;
                  })()}
                </TableCell>
